...
function getProductData(productNumber) {
  const nameInput = document
    .getElementById(`name${productNumber}`)
    .value.trim();

  const priceRaw = document.getElementById(`price${productNumber}`).value;

  const quantityRaw = document.getElementById(`quantity${productNumber}`).value;

  const unitInput = document
    .getElementById(`unit${productNumber}`)
    .value.toLowerCase();
